...
- 별도 사용자 `ServiceKey` 없이 hosted `k-skill-proxy` 로 조회

## 먼저 필요한 것

- [공통 설정 가이드](../setup.md) 확인
- 기본 hosted 경로: `https://k-skill-proxy.nomadamas.org`
- override 가 필요할 때만 `KSKILL_PROXY_BASE_URL`
...
1. client/skill 은 hosted `k-skill-proxy` 의 `/v1/han-river/water-level` endpoint 를 호출한다.
...
- hosted proxy 대신 self-host proxy 를 쓰려면 `KSKILL_PROXY_BASE_URL` 을 override 한다.
